...
function export_power_flow(system, variable, expressions, save_dirs; kwargs...)
    power_flow_mode = get(kwargs, :power_flow, VARIABLE_VALUE_BASED)        
...
function export_net_demand(results, parameters, save_dir; kwargs...)
    # TODO: add storage and pumped hydro charging profiles
    load_mapping = Dict()
    load_df = Dict()
...
function export_installed_capacity(system::PSY.System, save_dir; kwargs...)
    ...
    for gen_type in [PSY.ThermalGen, PSY.RenewableGen, PSY.HydroGen, PSY.Storage, PSY.HybridSystem]
        for g in get_components(gen_type, system, PSY.get_available)
...
function export_system_metadata(sys, save_dir; kwargs...)
    save_path = joinpath(dirname(save_dir), "metadata.json")
    if isfile(save_path)
    ...
    for l in PSY.get_components(PSY.Branch, sys)
        if typeof(l) <: DCBranch
            metadata["Lines"][l.name] = Dict(
                ...
                "from_region" => PSY.get_name(PSY.get_area(PSY.get_from(PSY.get_arc(l)))),
                "to_region" => PSY.get_name(PSY.get_area(PSY.get_to(PSY.get_arc(l)))),
            )
        else
            metadata["Lines"][l.name] = Dict(
                "from" => PSY.get_number(PSY.get_from(PSY.get_arc(l))), 
...
function export_marmot_inputs(results::PSI.ProblemResults, save_dir; kwargs...)
    ...
    export_power_flow(variables, expressions, save_dir; kwargs)

    export_net_demand(results, parameters, save_dir; kwargs)
    export_installed_capaicty(system, save_dir; kwargs)
end
...
function export_marmot_inputs(results::PSY.System, save_dir; kwargs...)
    preset_system_unit_settings!(system)
    export_installed_capaicty(system, save_dir; kwargs)
end
